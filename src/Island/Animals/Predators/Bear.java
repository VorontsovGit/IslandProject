...
public class Bear extends Predators{
    ...
    public static int multiply(HashSet arrayList) {
        long count = arrayList.stream().filter(x -> x instanceof Bear).count();
        int returnedCount = 0;
                    ...
                    while (iterator.hasNext()){
                         ...
                         */
                            ...
                            switch (moveDirection) {
                                case 0 -> {
                                    ((Bear) o).delete(island.getLocation(i, j));
                                    island.getLocation(i - moveSpeed, j).add(new Bear());
                                }
                                case 1 -> {
                                    ((Bear) o).delete(island.getLocation(i, j));
                                    island.getLocation(i, j + moveSpeed).add(new Bear());
                                }
                                case 2 -> {
                                    ((Bear) o).delete(island.getLocation(i, j));
                                    island.getLocation(i + moveSpeed, j).add(new Bear());
